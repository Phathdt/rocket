 ...
 * platforms, so pickup/dropoff use lettered pins and the driver uses an
 * inline SVG car puck with a pulsing "live tracking" ring.
 */

const BRAND_ORANGE = '#f97316';
const BRAND_BLUE = '#2563eb';

/** Circular lettered pin (A = pickup, B = dropoff). */
...
/** Driver puck with a pulsing ring conveying live position updates. */
export const driverIcon = L.divIcon({
  className: 'driver-marker',
  html: `<div style="position:relative;width:44px;height:44px;">
    <div class="driver-marker-pulse" style="position:absolute;inset:0;border-radius:9999px;background:${BRAND_BLUE};"></div>
    <div style="
      position:absolute;inset:6px;display:flex;align-items:center;justify-content:center;
      border-radius:9999px;background:${BRAND_BLUE};border:2px solid #fff;
